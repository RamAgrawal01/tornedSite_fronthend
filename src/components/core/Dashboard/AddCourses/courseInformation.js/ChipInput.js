...
export default function ChipInput({ label, name, placeholder, register, errors, setValue, }) {
  ...
  return (
    <div className="flex flex-col space-y-2">
      ...
      <label className="text-sm text-navyblue-50" htmlFor={name}>
      ...
      <div className="flex w-full flex-wrap gap-y-2">
        {chips?.map((chip, index) => (
          <div
            key={index}
            className="m-1 flex items-center rounded-full bg-pink-400 px-2 py-1 text-sm text-white font-semibold"
          >
            {chip}
